...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnVoiceInput) {
    btnVoiceInput.addEventListener("click", () => {
      alert("音声認識は後で追加できます。今はテキスト入力で壁打ちしてください。");
    });
  }
  if (btnVoiceStop) {
  ...
  return `
    <div class="lane-col mb-3">
      ...
      <div id="mid-${slug(big)}-${slug(mid)}" style="display:none; padding: 12px 0 0 0;">
        ...
        ${merged.length ? merged.map((p, i) => renderPostCard(p, "新統合")).join("") : `<div class="text-muted small mb-3">新統合はありません。</div>`}

        <div class="mb-2">${labelBadge("新提案")} ${proposals.length}</div>
        ${proposals.length ? proposals.map((p, i) => renderPostCard(p, "新提案")).join("") : `<div class="text-muted small mb-3">新提案はありません。</div>`}
